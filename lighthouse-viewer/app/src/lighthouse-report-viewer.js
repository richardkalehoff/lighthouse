...
class LighthouseViewerReport extends LighthouseReport {
  ...
  _addEventListeners() {
    ...
    if (gistURLInput) gistURLInput.addEventListener('change', this.onInputChange);
  }
  ...
  _validateReportJson(reportJson) {
    if (!reportJson.lighthouseVersion) throw new Error('JSON file was not generated by Lighthouse');
  ...
  listenForMessages() {
    ...
    if (self.opener && !self.opener.closed) self.opener.postMessage({opened: true}, '*');
  }
}
